        ...
        for (VehicleAd ad : adList) {
            if (ad.getVehicleTypeByPurpose().equals(vehicleType)) {
                ...
                System.out.println("Объявление № " + ad.getId() + " не прошло фильтр: тип авто - " +
                        vehicleType.getTypeName() + ", критерий " + vehicleType.getAttributeOfType() + ", " +
    ...
    public void filterByVehicleTypeByBodyTypes(VehicleTypeByBodyTypes vehicleType) {
        for (VehicleAd ad: adList) {
            if (ad.getVehicleTypeByBodyTypes().equals(vehicleType)) {
                ...
                System.out.println("Объявление № " + ad.getId() + " не прошло фильтр: тип кузова - " +
                        vehicleType.getTypeName() + ", критерий " + vehicleType.getAttributeOfType() + ", " +
                        " так как имеет тип кузова " + ad.getVehicleTypeByBodyTypes().getTypeName());
            }

        }
    }
    public void filterByVehicleTypeByFuelTypes(VehicleTypeByFuelType vehicleType) {
        for (VehicleAd ad: adList) {
            if (ad.getVehicleTypeByFuelType().equals(vehicleType)) {
                ...
                System.out.println("Объявление № " + ad.getId() + " не прошло фильтр: тип топлива - " +
                        vehicleType.getTypeName() + ", критерий " + vehicleType.getAttributeOfType() + ", " +
                        " так как имеет тип топлива " + ad.getVehicleTypeByFuelType().getTypeName());
            }
        }
    }
}
